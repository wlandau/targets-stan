...
If we implemented the model in `stan/model.stan` correctly, then roughly 95% of model fits should cover the true `beta` parameter that generated the data in 95% credible intervals.

```{r}
mean(fit_favorable$cover_beta)
```

The posterior median of `beta` should be reasonably close to the true value.

```{r}
ggplot(fit_favorable) +
